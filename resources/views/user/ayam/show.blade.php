    ...
    <div class="col-span-2 bg-white rounded-lg shadow p-6">
        <div class="flex items-start space-x-4">
            ...
            <div class="flex-1">
                ...
                <p class="text-gray-600">Umur: {{ $ayam->umur_hari ?? 0 }} hari</p>
                ...
                <p class="text-gray-600">Tanggal Masuk: {{ isset($ayam->tanggal_masuk) ? $ayam->tanggal_masuk->format('d/m/Y') : '-' }}</p>
                @if(isset($ayam->tanggal_produksi) && $ayam->tanggal_produksi)
                    <p class="text-gray-600">Tanggal Produksi: {{ $ayam->tanggal_produksi->format('d/m/Y') }}</p>
                @endif
                @if(isset($ayam->produksi_telur_per_minggu) && $ayam->produksi_telur_per_minggu > 0)
                    <p class="text-gray-600">Produksi Telur: {{ $ayam->produksi_telur_per_minggu }} butir/minggu</p>
                @endif
                <p class="text-gray-600 text-sm mt-2">Dibuat: {{ isset($ayam->created_at) ? $ayam->created_at->format('d/m/Y H:i') : '-' }}</p>
                @if(isset($ayam->keterangan) && $ayam->keterangan)
