...
@section('header', 'Tambah Pakan Baru')

@section('content')
<div class="bg-white rounded-lg shadow p-6">
    <form action="{{ route('admin.pakan.store') }}" method="POST" enctype="multipart/form-data">
        @csrf
        
        <div class="grid grid-cols-2 gap-4">
            <div>
                <label class="block text-sm font-medium mb-1">Kode Pakan *</label>
                <input type="text" name="kode_pakan" value="{{ old('kode_pakan') }}" 
            ...
            <div>
                ...
                <select name="jenis" class="w-full border rounded px-3 py-2 @error('jenis') border-red-500 @enderror">
                    <option value="konsentrat" {{ old('jenis') == 'konsentrat' ? 'selected' : '' }}>Konsentrat</option>
                    <option value="jagung" {{ old('jenis') == 'jagung' ? 'selected' : '' }}>Jagung</option>
                    <option value="dedak" {{ old('jenis') == 'dedak' ? 'selected' : '' }}>Dedak</option>
                    <option value="premix" {{ old('jenis') == 'premix' ? 'selected' : '' }}>Premix</option>
            ...
            <div>
                ...
                <input type="number" name="stok_minimal" value="{{ old('stok_minimal', 10) }}" 
