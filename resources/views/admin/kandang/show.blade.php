...
<div class="grid grid-cols-3 gap-6">
    <div class="col-span-2 bg-white rounded-lg shadow p-6">
        <div class="flex items-start space-x-4">
            <div>
                @if($kandang->foto)
                    <img src="{{ asset('storage/kandang/' . $kandang->foto) }}" 
                         alt="{{ $kandang->nama }}" 
            ...
            <div class="flex-1">
                ...
                <p class="text-gray-600">Kapasitas: {{ $kandang->kapasitas ?? 0 }} ekor</p>
                <p class="text-gray-600">Ayam Aktif: {{ $kandang->jumlah_ayam_aktif ?? 0 }} ekor</p>
                <p class="text-gray-600">
                    ...
                    <span class="px-2 py-1 text-xs rounded 
                        ...
                        {{ $kandang->status_label ?? 'Tidak Diketahui' }}
                    </span>
                </p>
                <p class="text-gray-600 mt-2">Dibuat oleh: {{ $kandang->creator->name ?? '-' }}</p>
                <p class="text-gray-600 text-sm">Dibuat: {{ isset($kandang->created_at) ? $kandang->created_at->format('d/m/Y H:i') : '-' }}</p>
    ...
    <div class="bg-white rounded-lg shadow p-6">
        ...
        @if(isset($kandang->ayams) && $kandang->ayams->count() > 0)
            <ul class="space-y-2">
                @foreach($kandang->ayams->take(5) as $ayam)
                    <li class="flex justify-between items-center border-b pb-1">
                        ...
                        <span class="text-xs px-2 py-1 rounded 
                            ...
                            {{ $ayam->status_kesehatan_label ?? 'Tidak Diketahui' }}
                        </span>
                    </li>
                @endforeach
